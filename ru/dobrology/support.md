---
title: Поддержать
description: Корм, лекарства, лежанки и игрушки — каждая вещь помогает животным жить лучше.
blocks:
  - title: ... Вещами
    list: 
      - name: Лакомства
        image: /assets/webp/yummy.webp
        description: Должен быть в закрытой заводской упаковке и с действующим сроком годности.
        
      - name: Лежанки и миски
        image: /assets/webp/catch.webp
        description: Подойдут и уже использованные в хорошем состоянии.

      - name: Лечебный корм
        image: /assets/webp/dish.webp
        description: Должен быть в закрытой заводской упаковке и с действующим сроком годности.

      - name: Игрушки
        image: /assets/webp/ball.webp
        description: Подойдут и уже использованные в хорошем состоянии.

      - name: Лекарства
        image: /assets/webp/medicines.webp
        description: Должен быть в закрытой заводской упаковке и действующим сроком годности.
    type: "cubes"
  
  - title: ... Делами
    description: Передать необходимое можно лично или оформив доставку. Режим работы приютов и номера телефонов для уточнения деталей можно нацти в карточках опекуна или карточках приюта.
    list: 
      - name: Приучить к поводку и ошейнику.
        image: /assets/webp/collar.webp
        description: Помогите питомцу освоиться на прогулке — это сделает его увереннее и спокойнее.
    
      - name: Потренировать базовые команды
        image: /assets/webp/whistle.webp
        description: «Сидеть», «Лежать», «Ко мне» — простые команды помогут питомцу быстрее найти новый дом.
    
      - name: Поиграть и научить контакту с человеком
        image: /assets/webp/medicines.webp
        description: Игры и общение помогают питомцу привыкнуть к людям и стать более социальным.
        
      - name: Создать контент о питомце
        image: /assets/webp/medicines.webp
        description: Сделайте фото/видео, напишите рассказ о характере собаки — это поможет ей найти любящую семью.
    type: "cubes"

  - title: ... Словами
    description: Делитесь ссылкой на сервис в социальных сетях! Один репост — и кто-то найдет дом, а кто-то — верного друга.
    image: /assets/webp/dogWithPhone.webp
    list: 
      - link: «ВКонтакте»
        url: https://vk.com/share.php?url=https://mikhaildadaev.github.io/petly
      
      - link: «Телеграм»
        url: https://t.me/share/url?url=https://mikhaildadaev.github.io/petly

    type: "social"

---

# Поддержать

<StyleBlockPage type="cubes"/>

<StyleBlockPage type="social"/>

<StyleImagePage src="/assets/webp/dogWithGirl.webp" width="320" float="center"/>